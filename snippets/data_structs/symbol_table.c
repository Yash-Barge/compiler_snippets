 ...
 */
int hash_function(ENUM_TOKENS tok, char* key, int capacity) {
...
struct symbol_table* symbol_table_new(int initial_capacity) {
    ...
    st->map = malloc(sizeof(struct head*) * initial_capacity);
 ...
 * @param key The Lexeme
 */
void symbol_table_insert(struct symbol_table* hm, char* key, ENUM_TOKENS tk) { // Double pointer as we may need to resize it
