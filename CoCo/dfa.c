...
TOKEN* createToken(){
    ...
    // token->token = (char *)calloc(50, sizeof(char));
    token->isError = false;
    return token;
}
    ...
    while(!fin){
        switch(state){
            ...
            case 33:
                ...
                //set state=0 and continue or exit?
                break;                           
            case 60:
                ch=getChar(io, state==0);
                switch(ch){
...
/*
to print or display TK_LE, use:

printf("%s", tokenenumnames[TK_LE]);

or 

enum token_names x=TK_LE;
printf("%s", tokenenumnames[x]);
